...
import type { Order, Client, Message, Testimonial, OrderStatus, OrderHistoryEntry, Cupom } from '../types';
...
interface AppContextType {
  ...
  // Cupons
  validateCoupon: (code: string) => Promise<Cupom | null>;
  ...
  try {
    ...
    if (data) {
      ...
      return {
        ...
        cupons: parsed.cupons || [],
      };
    }
  } catch { /* ignore */ }
  return { orders: [], clients: [], messages: [], testimonials: [], chatMessages: [], chatSessions: [], cupons: [] };
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  // Fetch chat messages and sessions from Supabase
  const fetchChatData = useCallback(async () => {
    try {
      ...
      if (msgs) {
        const chatMessages = msgs.map((m: any) => ({
          id: m.id,
      ...
      if (sessions) {
        const chatSessions = sessions.map((s: any) => ({
          id: s.id,
          ...
          status: s.status,
          unreadByAgent: s.unread_by_agent,
          unreadByClient: s.unread_by_client,
        })) as ChatSession[];
        setState(prev => ({ ...prev, chatSessions }));
      }
    } catch (err) {
      console.warn('Supabase fetch chat data failed:', err);
    }
  }, []);

  // Fetch cupons from Supabase
  const fetchCupons = useCallback(async () => {
    try {
      ...
      if (cupons) {
        ...
        setState(prev => ({ ...prev, cupons: mapped }));
      }
    } catch (err) {
      console.warn('Supabase fetch cupons failed:', err);
    }
  }, []);

  // Validate a coupon code
  const validateCoupon = useCallback(async (code: string): Promise<Cupom | null> => {
    try {
  ...
  const markCouponUsed = useCallback(async (code: string) => {
    try {
      await supabase.from('cupons').update({ usado: true }).eq('codigo', code.trim().toUpperCase());
      fetchCupons();
    } catch (err) {
  ...
  useEffect(() => {
    ...
    // Chat messages fetch from Supabase
    try {
      const chatMessagesUnsub = (supabase as any)
        .channel('public:chat_messages')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'chat_messages' }, () => {
          fetchChatData();
        })
        .subscribe();
      unsubscribes.push(() => chatMessagesUnsub.unsubscribe());
    } catch (e) {
      console.warn('Failed to register chat messages listener:', e);
    }

    // Chat sessions fetch from Supabase
    try {
      const chatSessionsUnsub = (supabase as any)
        .channel('public:chat_sessions')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'chat_sessions' }, () => {
          fetchChatData();
        })
        .subscribe();
      unsubscribes.push(() => chatSessionsUnsub.unsubscribe());
    } catch (e) {
  ...
  const sendChatMessage = useCallback(async (msg: Omit<ChatMessage, 'id' | 'timestamp'>) => {
    ...
    try {
      ...
      await supabase.from('chat_sessions').upsert({
        id: msg.sessionId,
        client_name: msg.clientName || 'Cliente Anónimo',
        last_message: msg.text,
        last_timestamp: new Date().toISOString(),
        status: msg.sender === 'user' ? 'bot' : 'active',
        unread_by_agent: msg.sender === 'user',
        unread_by_client: msg.sender === 'agent',
      }, { onConflict: 'id' });

    } catch (error) {
      console.error('Error sending chat message to Supabase:', error);
    }
  }, []);
  ...
  const requestAgent = useCallback(async (sessionId: string, clientName: string, email?: string, phone?: string) => {
    ...
    try {
      ...
      await supabase.from('chat_sessions').upsert({
        ...
        client_email: email || '',
        client_phone: phone || '',
        last_message: 'Pedido de falar com atendente iniciado.',
        last_timestamp: new Date().toISOString(),
        status: 'waiting',
      ...
      console.error('Error requesting agent via Supabase:', error);
    }
  }, []);
  ...
  const replyAsAgent = useCallback(async (sessionId: string, text: string) => {
    ...
    try {
      ...
      console.error('Error replying as agent via Supabase:', error);
    }
  }, []);
  ...
  const closeChatSession = useCallback(async (sessionId: string) => {
    ...
    try {
      ...
      await supabase.from('chat_sessions').update({
        status: 'closed',
        last_message: 'Conversa encerrada pelo atendente.',
        last_timestamp: new Date().toISOString(),
      }).eq('id', sessionId);

      // 2. Closed system message
      await supabase.from('chat_messages').insert([{
        session_id: sessionId,
        sender: 'bot',
        text: 'Esta conversa foi encerrada pelo atendente. Se precisar de algo mais, sinta-se à vontade para reabrir ou iniciar um novo chat!',
        timestamp: now,
      }]);

    } catch (error) {
      console.error('Error closing chat session via Supabase:', error);
    }
  }, []);

  const markChatReadByAgent = useCallback(async (sessionId: string) => {
    try {
      await supabase.from('chat_sessions').update({ unread_by_agent: false }).eq('id', sessionId);
    } catch (error) {
  ...
  const markChatReadByClient = useCallback(async (sessionId: string) => {
    try {
      await supabase.from('chat_sessions').update({ unread_by_client: false }).eq('id', sessionId);
    } catch (error) {
