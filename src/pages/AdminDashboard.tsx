...
import {
  LayoutDashboard, ClipboardList, Users, MessageSquare, Package, Settings, Ticket,
...
type AdminTab = 'dashboard' | 'orders' | 'clients' | 'messages' | 'services' | 'settings' | 'chatbot' | 'cupons';
...
export default function AdminDashboard() {
  ...
  // ⚽ Copa do Mundo Theme Toggle
  const [copaTheme, setCopaTheme] = useState(() => localStorage.getItem('mundodedoces_admin_copa') === 'true');
  ...
  const handleStatusUpdate = async (orderId: string, status: OrderStatus) => {
    ...
    if (isNotifyEnabled) {
      ...
      if (order) {
        ...
        // Enviar notificação push local (se a app estiver instalada)
        const { sendLocalNotification } = await import('../capacitor/notifications');
        await sendLocalNotification(
          `Encomenda ${order.orderNumber}`,
          `Estado atualizado para: ${statusLabel}`
        );
      }
    }
  ...
  return (
      <div className={`min-h-screen ${copaTheme ? 'bg-[#0a0a1a]' : 'bg-gray-900'}`}>
      <div className="flex">
        {/* Sidebar */}
        <aside className={`hidden lg:flex lg:flex-col lg:w-64 ${copaTheme ? 'bg-[#001a4d] border-[#009c3b]' : 'bg-gray-800 border-gray-700'} min-h-screen border-r`}>
          <div className={`p-6 border-b ${copaTheme ? 'border-[#009c3b]/30' : 'border-gray-700'} space-y-2`}>
            <div className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${copaTheme ? 'from-yellow-400 to-green-500' : 'from-rosa-400 to-rosa-600'} flex items-center justify-center`}>
            ...
            <button onClick={() => {
              ...
              setCopaTheme(next);
              localStorage.setItem('mundodedoces_admin_copa', String(next));
            }} className={`w-full px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all flex items-center justify-center gap-1.5 ${copaTheme ? 'bg-yellow-400 text-[#002776]' : 'bg-gray-700 text-gray-400 hover:bg-gray-600'}`}>
              {copaTheme ? '⚽ Modo Copa Ativo' : '⚽ Ativar Modo Copa'}
            </button>
          </div>
            ...
            {tabs.map(tab => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                  activeTab === tab.key
                    ? copaTheme ? 'bg-yellow-400/10 text-yellow-400 border border-yellow-400/20' : 'bg-rosa-500/10 text-rosa-400 border border-rosa-500/20'
          ...
          <div className={`p-4 border-t space-y-2 ${copaTheme ? 'border-[#009c3b]/30' : 'border-gray-700'}`}>
        ...
        {/* Mobile nav */}
        <div className={`lg:hidden fixed bottom-0 left-0 right-0 z-50 ${copaTheme ? 'bg-[#001a4d] border-[#009c3b]' : 'bg-gray-800 border-gray-700'} border-t`}>
          <div className="flex">
            {tabs.map(tab => (
              <button
                key={tab.key}
                ...
                className={`flex-1 flex flex-col items-center py-3 text-xs font-medium transition-all ${
                  activeTab === tab.key ? (copaTheme ? 'text-yellow-400' : 'text-rosa-400') : 'text-gray-400'
                }`}
              >
                {tab.icon}
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 pb-20 lg:pb-8">
          ...
          {activeTab === 'dashboard' && (
            <div>
              <h1 className={`text-2xl font-bold ${copaTheme ? 'text-yellow-300 flex items-center gap-2' : 'text-white'} mb-6`}>
                {copaTheme && <span className="text-2xl">⚽</span>}Dashboard
              </h1>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                {[
                  { label: 'Total Encomendas', value: state.orders.length, icon: ClipboardList, color: 'bg-blue-500', copaColor: 'bg-[#009c3b]' },
                  { label: 'Novas Mensagens', value: unreadMessages, icon: MessageSquare, color: 'bg-rosa-500', copaColor: 'bg-yellow-500' },
                  { label: 'Clientes', value: state.clients.length, icon: Users, color: 'bg-green-500', copaColor: 'bg-[#002776]' },
                  { label: 'Por Confirmar', value: state.orders.filter(o => o.status === 'recebida' || o.status === 'em-analise').length, icon: Clock, color: 'bg-dourado-500', copaColor: 'bg-[#ffdf00]' },
                ].map(stat => (
                  <div key={stat.label} className={`${copaTheme ? 'bg-[#001a4d] border-[#009c3b]/30' : 'bg-gray-800 border-gray-700'} border rounded-2xl p-5`}>
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-gray-400 text-sm">{stat.label}</span>
                      <div className={`w-10 h-10 rounded-xl ${copaTheme ? stat.copaColor : stat.color} flex items-center justify-center`}>
          ...
          {activeTab === 'cupons' && (
            <div>
              <div className="flex items-center justify-between mb-6">
                <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Ticket className="w-6 h-6 text-dourado-400" /> Cupons de Desconto</h1>
                <span className="text-xs text-gray-400 bg-gray-800 border border-gray-700 rounded-xl px-3 py-1.5">Total: {state.cupons.length} cupons</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                {[{ label: 'Ativos', value: state.cupons.filter(c => !c.usado).length, color: 'bg-green-500' },{ label: 'Usados', value: state.cupons.filter(c => c.usado).length, color: 'bg-amber-500' },{ label: 'Total', value: state.cupons.length, color: 'bg-blue-500' }].map(s => (
                  <div key={s.label} className="bg-gray-800 border border-gray-700 rounded-2xl p-5">
                    <div className="flex items-center justify-between mb-3"><span className="text-gray-400 text-sm">{s.label}</span><div className={`w-10 h-10 rounded-xl ${s.color} flex items-center justify-center`}><Ticket className="w-5 h-5 text-white" /></div></div>
              ...
              <div className="bg-gray-800 border border-gray-700 rounded-2xl overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="text-left text-gray-400 border-b border-gray-700"><th className="p-4 font-medium">Código</th><th className="p-4 font-medium">Desc</th><th className="p-4 font-medium">Estado</th><th className="p-4 font-medium">Expira</th></tr></thead>
                    <tbody>
                      {state.cupons.map(c => (
                        <tr key={c.id} className="border-b border-gray-700/50">
                          <td className="p-4"><code className="text-white font-mono font-bold bg-gray-750 px-2 py-1 rounded-lg text-xs">{c.codigo}</code></td>
                          <td className="p-4"><span className="text-green-400 font-bold text-sm">{c.desconto}%</span></td>
                          <td className="p-4"><span className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${c.usado ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-green-500/10 text-green-400 border-green-500/20'}`}>{c.usado ? 'Usado' : 'Ativo'}</span></td>
                          <td className="p-4 text-gray-400 text-xs">{new Date(c.expira_em).toLocaleDateString('pt-PT')}</td>
                        </tr>
                      ))}
                      {state.cupons.length === 0 && <tr><td colSpan={4} className="p-8 text-center text-gray-500 text-xs">Nenhum cupom.</td></tr>}
                    </tbody>
                  </table>
                </div>
              </div>
